...
public class RxNowWeatherService {
    private volatile static RxNowWeatherService apiRetrofit;
    private Retrofit mRetrofit;
    private static final int READ_TIMEOUT = 60;//读取超时时间,单位秒
    private static final int CONN_TIMEOUT = 50;//连接超时时间,单位秒

    private RxNowWeatherService(){
    ...
    public static RxNowWeatherService getInstance(){
        if (apiRetrofit == null){
            synchronized (Object.class){
                if (apiRetrofit == null){
                    apiRetrofit = new RxNowWeatherService();
                }
            }
        }
        return apiRetrofit;
